...
(function(µ,SMOD,GMOD,HMOD,SC){
	...
	let backGardener=module.exports={
		...
		handleRequest:function(context,request,response,requestPath)
		{
			...
			if(app!=null&&app.state===SC.AbstractWorker.states.OPEN)
			{
				...
				{//restService
					...
					function(error)
					{
						logger.error({lnr:"008",path:requestPath.slice(1),error:error},`error while calling rest service of /${context}`);
		...
		startApp:function(sessionToken,context)
		{
			return backGardener.callModule("permissions","check",[sessionToken,["startApp"]])
			.then(function()
			{
				let app=SC.backyard.get(context);
				if(!app) return Promise.reject("no such app ("+context+")");
				if(app.state!==SC.AbstractWorker.states.CLOSE) return Promise.reject("app is not closed");
				return app.restart();
			});
		},
		stopApp:function(sessionToken,context)
		{
			return backGardener.callModule("permissions","check",[sessionToken,["stopApp"]])
			.then(function()
			{
				let app=SC.backyard.get(context);
				if(!app) return Promise.reject("no such app ("+context+")");
				if(app.state!==SC.AbstractWorker.states.OPEN) return Promise.reject("app is not open");
				return app.stop();
			});
		},
		removeApp:function(sessionToken,context)
		{
			return backGardener.callModule("permissions","check",[sessionToken,["removeApp"]])
			.then(function()
			{
				let app=SC.backyard.get(context);
				if(!app) return Promise.reject("no such app ("+context+")");
				SC.backyard.remove(context);
				app.destroy();
				return SC.backyard.setAppConfig(sessionToken,context,null);
			});
		},
		checkCommunication:async function(contextFrom,contextTo)
		{
			let appFrom=SC.backyard.get(contextFrom);
			let appTo=SC.backyard.get(contextTo);
			if(!appFrom.niwaConfig.name) throw `app under ${appFrom} has no name`;
			if(!appTo) throw `no app under ${contextTo}`;
			if(appTo.state!==SC.AbstractWorker.states.OPEN) throw `app under ${context} is not running`;
			if(!appTo.niwaConfig.name) throw `app under ${appTo} has no name`;

			appsConfig=await SC.backyard.appsConfig;
			let test=function(a,b)
			{
				if(!(a in appsConfig)) return config.allowCommunication;
				if(!( "communication" in appsConfig[a])) return config.allowCommunication;
				if(typeof appsConfig[a].communication=="boolean") return appsConfig[a].communication
				if(typeof appsConfig[a].communication==null) return config.allowCommunication;
				if(!(b in appsConfig[a].communication)) return config.allowCommunication;
				if(typeof appsConfig[a].communication[b]=="boolean") return appsConfig[a].communication[b]
				if(appsConfig[a].communication[b]==null) return config.allowCommunication;
				return appsConfig[a].communication[b].includes(b);
			};

			if(!test(contextFrom,contextTo))
			{
				throw "communication not allowed ${contextFrom} -> ${contextTo}";
			}
			if(!test(contextTo,contextFrom))
			{
				throw "communication not allowed ${contextTo} -> ${contextFrom}";
			}
