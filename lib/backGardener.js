...
(function(µ,SMOD,GMOD,HMOD,SC){
	...
	let backGardener=module.exports={
		...
		callModule:function(moduleName,method,args=[])
		{
			...
			{
				logger.error({error:e},"error loading module");
				return Promise.reject("error loading module");
			}

			if(!(method in module))
			{
				return Promise.reject(`no method ${method} in module ${moduleName}`);
			}
			try
			{
			...
			{
				logger.error({error:e},`error calling method ${method} of module ${moduleName}`);
				return Promise.reject(`error calling method ${method} of module ${moduleName}`);
			}
		}
	};
	/**
