...
(function(µ,SMOD,GMOD,HMOD,SC)
{
	...
	let Field=µ.Class(Worker,{
		...
		}={})
		{
			...
			this.addEventListener("workerMessage",function(message)
			{
			...
			try
			{
				switch (fieldPart[0])
				{
					case "rest":
						...
						break;
					case "event":
						await this.guideEvent(request,response, fieldPart.slice(1));
						return;
						break;
					default: //resource
		...
		async guideEvent(request, response, fieldPart)
		{
			let eventName=fieldPart.join("/");
			let data = await this.request("getEventSourceData", [eventName]);
			if(!data)
			{
				SC.fillResponse(response,new SC.ServiceResult({status:404,data:"no such EventSource"}));
			}
...
`			);
				...
				if(!this.eventSourceMap.has(eventName))
				{
					this.eventSourceMap.set(eventName,[]);
				}
				let responses=this.eventSourceMap.get(eventName);
				responses.push(response);
				response.connection.addListener("close", function ()
				{
					let index=responses.indexOf(response);
					if(index===-1)this.logger&&this.logger.error("could not find response in eventSources");
