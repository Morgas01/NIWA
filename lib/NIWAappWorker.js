...
worker.init=function(param)
{
	...
	.then(function(niwaConfig)
	{
		...
		return p.then(worker.startRestServices)
		.then(()=>niwaConfig);
	})
	.catch(function(e)
	{
...
worker.startRestServices=function()
{
...
worker.eventSource=function(name,getter)
{
	...
	{
		worker.send({type:"triggerEventSource",name:name,event:eventName,data:data});
	};
...
{
	var eventSource=worker.eventSourcesMap.get(name);
