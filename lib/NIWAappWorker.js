...
µ.getModule("Worker/BaseWorker"); // NodeJs/Worker/BaseWorker.js
...
worker.initialize.then(function(param)
{
	...
	µ.setModule("niwaWorkDir",param.niwaWorkDir);
	µ.setModule("niwaAppWorkDir",param.niwaAppWorkDir);
	let LOG=require("./logger");
	LOG.setLevel(param.logLevel);
	worker.logger=LOG("apps/"+param.context);
	LOG.setCoreLogger(worker.logger);
});
...
//util methods
worker.ask=function(context,method,data,timeout)
{
	return worker.feedback({context:context,method:method,data:data},timeout);
};
worker.module=function(moduleName,methodName,args=[])
{
	...
	.then(SC.ServiceResult.parse,SC.ServiceResult.parseThrow);
};
worker.getCommunicationList=function(name)
{
	return worker.ask("NIWA","communicationList",{name:name});
};

//callback
worker.serverStarted=function()
{
	// overwrite for callback when server is started
	// and communication between apps is possible
}