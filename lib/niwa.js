...
let Niwa=µ.Class({
	...
	}={})
	{
		...
		this.name =					name!=null?name								:(config.name!=null?config.name								:Niwa.DEFAULT_NAMES[Math.round(Math.random()*Niwa.DEFAULT_NAMES.length)%Niwa.DEFAULT_NAMES.length]);
		...
		this.logger=SC.logger(this.yard,"niwa",this.name,this.logLevel);
		if (configError)
		{
			this.logger.error({error:configError},"could not load config");//TODO move config to bin/niwa?
		}
		...
		try {
			...
			{
				fieldPaths=fieldPaths.filter(fieldPath=>!config.fields.find(f=>SC.path.resolve(this.yard,f.path)===fieldPath));
			}
		...
		{
			f.path=SC.path.resolve(this.yard,f.path)
			f.shedBase=SC.path.resolve(this.yard,"shed");
			return new SC.Field(f);
		});
		for (let field of this.fields)
		{
			field.addEventListener("workerState",this._updateNeighbors,{scope:this});
		}
	},
	getConfig()
	{
		return {
			...
			fields:this.fields.map(f=>
			{
				return {
					...
					name:f.name,
					autoOpen:f.autoOpen,
					allowCommunication:f.allowCommunication,
	...
	async addField({path,name,autoOpen,allowCommunicatio,communicationList})
	{
		let fieldParam = {
			...
			logger:this.logger.child({field:name},true)
		};
	...
	getOwner()
	{
		let ownerLogger=this.logger.child({field:"owner"},true);
		return {
			name:"owner",
			type:["owner"],
			field: {
				helpNeighbor:async (method, args, timeout)=>
				{
					switch (method)
					{
						...
						case "addField":
						{
							...
							ownerLogger.info({fieldParam},"add Field");
							return this.addField(fieldParam);
						}
						case "removeField":
						{
							let name = args;
							ownerLogger.info({removed:name},"remove Field");
							return this.removeField(name);
						}
						case "openField":
						{
							let name = args;
							ownerLogger.info({opened:name},"open Field");
							return this.openField(name).then(µ.constantFunctions.n);
						}
						case "closeField":
						{
							let name = args;
							ownerLogger.info({closed:name},"close Field");
							return this.closeField(name).then(µ.constantFunctions.n);
						}
						default:
