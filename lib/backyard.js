		...
		appFeedback:function({name,method,data})
		{
			...
			if(name=="NIWA")
			{
				...
				switch (method) {
					...
					case "setAppConfig":
						return backyard.setAppConfig(data.context,daata.config);
					case "addApp":
						return new Promise(function(resolve)
						{
							SC.backGardener.addApp(data.context,{path:data.path,autoStart:false});
							resolve(backyard.setAppConfig(data.context,{path:data.path}));
						});
					case "startApp":
						return SC.backGardener.startApp(data);
					case "stopApp":
						return SC.backGardener.stopApp(data);
					case "removeApp":
						return SC.backGardener.removeApp(data);
					case "module":
