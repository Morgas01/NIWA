...
var backGarden=module.exports={
	initApps:function()
	{
		...
		.then(results=>
		{
			logger.info({results:results},"apps started ["+results.filter(r=>r.appState.state==="running").map(r=>r.name)+"]")
			//populate apps
			for(var result of results)
			{
				var nameDictionary={};
				ids:for(var id of [].concat(result.appState.data.dependencies||[],result.appState.data.uses||[]))
				{
	...
	startApp:SC.Promise.pledge(function(signal,name,path)
	{
		...
		activeApps.get(name).addListener(".readyState:once",null,e=>signal.resolve({name:name,appState:e.value}));
	}),
	handleRequest:function(appName,request,response,requestPath)
	{
		if(activeApps.has(appName))
		{
			...
			{//event source
				app.eventSource(request,cutQuery(requestPath.slice(1).join("/")),response);
			}
			else
			{//resource
				if(requestPath[requestPath.length-2]=="css"&&requestPath[requestPath.length-1].match(/^[^\?]+.less/))
				{//compile less file
					requestPath[requestPath.length-2]="less";
					SC.fillResponse(
						response,
						SC.less.render('@import "'+app.folder.clone().changePath(cutQuery(requestPath.join("/"))).getAbsolutePath()+'";')
						.then(data=>data.css,error=>Promise.reject(JSON.stringify(error))),
						{"Content-Type":SC.fillResponse.getMimeType(".css")}
					);
				}
				else SC.fillResponse(response,app.folder.clone().changePath(cutQuery(requestPath.join("/"))));
			}
			return true;
		}
		return false;
...
var cutQuery=function(url)
{
	return SC.url.parse(url).pathname;
}
/**
