(function(µ,SMOD,GMOD,HMOD,SC){
	...
	request.json("rest/user/config?token="+localStorage.getItem("NIWA_SESSION"))
	.then(function(data)
	{
		...
		SC.actionize({
            ...
            "setPassword":function()
            {
				...
				actionsContainer.disabled=true;
            	new SC.dlg(`<input type="hidden" name="token" value="${localStorage.getItem('NIWA_SESSION')}"><input name="password" autofocus>`)
            	.always(function()
            	{
            		actionsContainer.disabled=false;
            	});
